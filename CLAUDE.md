...
- `puffsat_sim/nav_feasibility.py` — pure C1 sweep harness (ADR 0012, no JVM): one-axis-at-a-time `NavFeasibilitySpec` grid (range σ / Doppler σ incl. range-only / cadence / cone geometry / n_nodes / Q), per-cell LinCov along the coast arc to apogee, Σ→RTN, `ΦΣΦᵀ` catch-radius verdict via C0's Φ; `sweep_nav_feasibility` + `format_nav_feasibility`.
