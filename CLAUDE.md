...
- `puffsat_sim/train.py` — pure train-mode dispersion core, the first Rung-D / D1 sub-slice (ADR 0016/0018, no JVM): the shared-vs-per-unit draw split — `TrainDispersionSpec` separates each `DispersionSpec` σ into a **shared** (per-train: coefficient bias / F10.7-Ap drivers / deployer systematic; the §16.7 common-density component) and a **per-unit** part (coefficient spread / injection scatter); `sample_train` composes the same `RunInputs` the JVM `run_record` consumes (so D1.1 wires in unchanged), `replay_train_unit` is the standalone §14.2 replay (shared from `train_index`, per-unit from `(train_index, unit_index)`); plus the train-relative reduction — `summarize_train_capture` → `TrainCaptureStats` splits a train's `guidance.PlateMiss` arrivals into **centroid drift** (vs the ±2 km `CENTROID_RETARGET_M`, `retarget_ok`) and **scatter about the centroid** (per-axis σ vs `guidance.CAPTURE_SIGMA_MAX_M`, `scatter_sigma_ok`), reporting `capture_about_centroid` (reusing `guidance.capture_fraction`) vs `capture_absolute`; `format_train_capture`. **D1.1 extension:** the Φ-composed hand-off entry offset — `sample_train_entry_offsets` / `replay_train_entry_offset` draw a per-unit 2-D lateral (⊥v) entry = shared centroid + per-unit scatter, magnitudes the characterized C0/C1/C2a budget legs (`ENTRY_LATERAL_PERUNIT_M` 141 m / `ENTRY_LATERAL_SHARED_M` 149 m, sampled isotropically at σ/√2; seed tree masked off the coefficient tree by `_ENTRY_SEED_MASK`); and the verdict surface `summarize_train_ensemble` → `TrainEnsembleFinding` (the D1.0 `TrainCaptureStats` + terminal-aim Δv `propellant.propellant_curve` vs <2 % + perigee diagnostic) / `format_train_ensemble`. **Entry re-run DONE — "over-stress 6×" hypothesis FALSIFIED, model UNCHANGED (ADR 0018, 2026-06-15):** the post-closeout claim that the 141 m (T, N) entry over-stresses the ⊥v cliff ~6× (true ⊥v 24.6 m, ~86 % along-track) was tested by the flown re-run and **withdrawn**. A ⊥v-only 24.6 m entry is *pessimistic* not accurate (3.2 µrad: σ 1.47→2.14 m, P 99→91 %): the C3b significance gate (`guidance.significant_zem`, keyed on the full 3-D ZEM) needs a signal to trip, and a *lone* small ⊥v sits at the gate threshold so the loop fires late. The **real entry is anisotropic** — ~172 m along-v (range-observable along the closing LOS → trips the gate) + ~24.6 m ⊥v — and restoring the along-v recovers σ **1.35 m / 100 %** at 3.2 µrad, **matching the committed 141 m-isotropic result** (σ 1.47 m): the isotropic proxy trips the gate via a mis-attributed ⊥v exactly as the real along-v does, so it is ≈accurate and mildly conservative. So 141/149 m **kept**; D1.1 conclusions stand — requirement ~3.2 µrad, single detector marginal at the operating point, **fusion the robust baseline** (5-array σ 0.88/100 %, +co-flyer σ 0.32/100 %, pooled 192 — **but see the entry-convention finding below: that "100 %/zero escapes" was a train-count artefact**). The genuinely accurate *anisotropic* entry (along-v + ⊥v) is an optional D1.x refinement (~identical outcome), not a correction. The (T, N) 141/149 figures stay correct throughout: acquisition-FOV delivery dispersion (`tracker_budget`), the C2a RSS budget ledger (`coeff_requirement`/`authority`), and the `corrector_validation` crossing proxy. **Entry-convention finding (ADR 0018, 2026-07-22) — "zero escapes" WITHDRAWN:** pooling 8 independent trains × 40 units = **320** at the 5-array grade measures **3 escapes → 99.06 %, one-sided 95 % LB 97.6 %** (the old "zero escapes in 192" was a *train-count* artefact — too few shared draws to sample a large one). All 3 escapes are **entry-cliff, not noise**: units whose hand-off entry passed the ~475–500 m funnel; arrivals are bimodal (captured core σ ≈ 0.5 m + tens-of-metre outliers) so a pooled σ is a mixture statistic, never read against the 1.65 m criterion. **Two conventions, 3 orders apart:** *as flown* the funnel nulls shared ⊕ per-unit (σ 144.8 m → escape ~1e-2), but ADR 0016 specifies the ±2 km centroid retarget absorbs the shared leg, leaving per-unit only (σ 141 m → escape ~1e-5, the basis of 99.999 %). `run_guidance` never got the retarget, so the flown pipeline is *conservative* — re-flying the two escaping trains with `sigma_entry_lateral_shared_m=0.0` gives **80/80, σ 0.58 m**. Verdict direction unchanged; the *claim* changed. Pure reduction: `PooledCaptureFinding`/`summarize_pooled_capture`/`binomial_lower_bound_95` (Clopper–Pearson)/`format_pooled_capture`.
...
- `puffsat_sim/guidance.py` — pure C3b terminal-guidance core (ADR 0014/0015, no JVM): ZEM law (`zem_acceleration`, `predicted_zem` on the two-body+J2 onboard model), the noise-discipline tick (`terminal_tick`: significance gate / track window / firing-lag hold — see the measured constants block), `TrackerGrade` σ_θ·R nav noise as a Gauss–Markov `NavNoiseProcess`, `slew_limited_direction`, plate-frame miss (`plate_frame_miss`/`PlateMiss`/`capture_fraction`), sweep value types (`GuidanceSweepSpec`/`GuidanceCell`/`TerminalGuidanceFinding`), `measured_catch_radius_m`, `format_terminal_guidance`. **Moving-target frame (ADR 0023):** `plate_frame_miss` takes an optional `target_velocity_m_s` (default 0 = the historical fixed-point frame, byte-identical) and scores ⊥ the closing velocity `v_rel = v − v_target`; `reflect_radial_velocity` builds the mirror-ascending placeholder target, `lateral_scatter_sigma_m` + `MovingTargetFinding`/`format_moving_target` reduce the fixed-vs-moving comparison. Measured (mirror target): closing speed ~3 km/s (interception ~8° from horizontal → co-moving), capture unaffected, fixed frame mildly conservative — but target-model-dependent. **The mirror is the *minimum*-closing corner, not a pessimistic one** (a faster co-directional target closes more gently): scaling `v_target = s·v_mirror` rises monotonically 3.19 → 10.78 km/s as `s` → 0, and `s = 0` *is* the fixed frame, so the committed numbers bound the whole co-directional family — but **not** head-on/cross-track geometries (up to ~21.6 km/s). Limitation: the loop is *scored* relative but still *flown* to a fixed aim point — **no target lead**, so ADR 0023 is a sensitivity study, not an end-to-end powered-target sim.
...
- `puffsat_sim/tracker_fusion.py` — pure multi-tracker fusion gate (ADR 0019, the early-error reduction; **no JVM**, builds on `tracker_budget.py`): D1.1 found the combined entry×noise stress needs an **effective** ~3 µrad terminal-nav grade; this quantifies how a multi-tracker architecture of redundant bench-grade (3.2 µrad-class) detectors recovers it with ~2× margin (NOT from 10 µrad hardware — five true 10 µrad fuse to 4.56 µrad and fail, the 10 µrad class needs N≈12; ADR 0019 correction 2026-07-09) by two levers — **averaging** (`angular_sigma_theta_rad` = √(σ_common² + σ_indep²/N): the independent part — distortion⊕gyro⊕photon, separately calibrated — averages with √N, the smear `SMEAR_COMMON_SIGMA_RAD` common-mode floor does not) and **range** (`lateral_sigma_m` = σ_θ·R ⊕ rel-geom floor; a closer `coflyer` at `COFLYER_RANGE_M` 500 km vs the target's `TARGET_RANGE_M` 2603 km has proportionally less lateral error, plus the GNSS-pinned `COFLYER_RELGEOM_SIGMA_M` rocket→target vector); `Tracker` value type, `target_array(n)` / `coflyer(n)` constructors, `fuse_lateral_sigma_m` (inverse-variance) → `effective_sigma_theta_rad` at the design range read against the D1.1 `D1_CAPTURE_GRADE_SIGMA_THETA_RAD` (3.2 µrad); `TrackerFusionFinding` (`meets_capture_grade`/`margin`/`homing_floor_m`) + `tracker_fusion_finding` (pure runner) + `format_tracker_fusion`. Measured: target 5-array alone → 1.62 µrad (2.0× inside, no phasing dependency); +co-flyer → 0.76 µrad (4.2×). **Stage 2 — the Lever-2 co-flyer phasing gate** (pure verdict; the JVM run is `runs/coflyer.py`): `CoflyerPhasing` / `phasing_verdict` reduce the rocket↔centroid range and rocket altitude sampled over the 800→200 km terminal window to a two-part feasibility — `range_ok` (peak separation ≤ the `COFLYER_RANGE_M` angle-useful design range, so the σ_θ·R lever the fusion credits actually holds) and `gps_ok` (peak rocket altitude ≤ `GPS_CEILING_M` 20 200 km, inside the unlocked-spaceborne-GNSS volume that pins the rocket→target vector independently of the long baseline) → `feasible`; `format_coflyer_phasing`. **The re-key (ADR 0019 decision 4)**: `fused_tracker_grade` collapses an architecture into the C3b/D1.1 loop's `guidance.TrackerGrade` (effective σ_θ + unchanged ranging σ — the noise model already consumed a scalar σ_θ, so fusion adds no noise code); the canonical architectures `single_target_detector` / `target_array_only` / `array_with_coflyer` (`ARRAY_N_DETECTORS` 5, `COFLYER_N_DETECTORS` 3) feed the D1.1 re-run.
- `puffsat_sim/distortion_field.py` — pure distortion-field study, the **terminal cross-track hedge** (grill 2026-06-15, `todos/improve_terminal_crosstrack.md`; **no JVM** like `tracker_budget.py` — a focal-plane question; **standalone scope A** — imports the canonical constants from `tracker_budget`/`tracker_fusion` and changes none): models the σ_θ budget's *scalar* 3 µrad `distortion_floor_rad` as a spatial **field** (RMS + correlation structure) to harden the load-bearing bench-calibration number — **defense-in-depth, NOT a capture gap** (D1 feasible past the noise knee, ADR 0021). One field, two readouts. **Readout A — differential ("Gaia trick") gain:** beacon-vs-reference-star bearing → residual `σ_d·√(2(1−ρ))` (`differential_residual_rad`, `gaussian_correlation` ρ(Δθ)=exp(−Δθ²/2L²), `nearest_star_separation_rad` 1/(2√n) = 2.99 mrad lag); **payoff sign-flips on the spectrum** — smooth/long-L cuts the floor (gain 10× at L=30 mrad), rough/short-L is √2 *worse* (gain 0.71×); the deliverable is the `differential_curve` + `breakeven_correlation_length_rad` (ρ=0.5 → L=Δθ/√(2 ln2) = **2.54 mrad**), which *outputs a bench-characterization requirement* (bank the gain only if measured L ≳ break-even). **Readout B — array common-mode tolerance** (audit of ADR 0019's zero-common-mode split): `array_sigma_theta_rad` with an equicorrelated detector-to-detector distortion fraction ρ (variance `σ²·(1+(N−1)ρ)/N`; only distortion carries ρ, smear is the common floor, gyro averages) → `common_mode_tolerance` solve. Measured: ρ=0 reproduces the committed **1.62 µrad** 5-array grade (optimistic end), ρ=1 (fully common-mode) collapses to **3.13 µrad** ≈ the single-detector grade → tolerance >1 = **ROBUST** (capture-grade across the entire correlation range), but the 2.0× √N margin is best-case only, so a physically independent platform (the co-flyer) is what restores margin under pessimistic distortion correlation. `DetectorBudget`/`DifferentialPoint`/`ArrayTolerancePoint`/`DistortionFieldFinding` value types + `distortion_field_finding` (pure runner) + `format_distortion_field`. The Readout-A analytic σ_diff is cross-checked against a literal Gaussian random-field draw in the tests. No ADR (reversible scoping); a live re-key of the fusion split would be warranted only if a bench measurement later shows L below break-even or material common-mode distortion.
- `puffsat_sim/centering_budget.py` — pure surveyor-anchored centering budget (ADR 0022, the deferred Tier-2 ~10 cm sizing; **no JVM** like `tracker_budget.py`/`distortion_field.py` — metrology/focal-plane, not orbit): sizes the paper's deferred terminal cm-centering follow-on (CONTEXT.md *Surveyor-anchored centering*; NOT a change to the committed 5 m Tier-1 baseline). The plate is the **RSS of two legs**, `plate = 3.03·√(σ_hoop² + (σ_θ·v/f)²)` (the ADR 0015 capture criterion): the hoop-pinned block-shift **bias** σ_hoop and the per-unit camera **scatter** σ_θ·v/f (`σ_δ = σ_θ·v/f`, the committed cadence formula — `STROBE_RATE_HZ` 2 Hz → 5.4 km link). The km-class link is made **distortion-limited, not photon-limited** by a `QSwitchedBeacon` (100 kW peak / 10 Hz / 300 mW avg → 30 mJ/300 ns pulse; the tracker-budget photon model consumes it as peak-power × gated-exposure = pulse energy) read through a `SOMEWHAT_DIRECTIONAL_HALFANGLE_RAD` 5° coarse cone + narrowband gate → photon σ_θ 0.18 µrad, ~17× under the 3 µrad floor; `CameraModel.hardware` derives the `TrackerHardware` and the N-camera ρ-reduction reuses `distortion_field.array_sigma_theta_rad` (ρ=1 identical share the bias, ρ=0 diverse average √N). `CenteringSpec`/`PlatePoint`/`CameraRead`/`ReferencePoint`/`CenteringFinding` value types + `hoop_sweep`/`reference_points`/`camera_reads` + `centering_finding` (pure runner) + `format_centering_budget`. Measured: nominal σ_hoop 1 cm ⊕ scatter 1.62 cm → 5.8 cm plate (87× off 5 m, 10 cm robust, tolerates σ_hoop ≤ 2.9 cm); **robust 5 cm needs both legs** (mm hoop alone = 4.99 cm, zero margin; + smaller scatter via 4 Hz / diverse cameras / lower floor — the scatter leg alone sets ~4.9 cm). **Stays sized, not simulated** (binders are bench characterizations, dynamics inside the 475 m funnel) → the simulated frontier stays the 5 m plate; next rigor is a bench test.
