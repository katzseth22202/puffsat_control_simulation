...
- `puffsat_sim/descent.py` — truth-path kernel (ADR 0017): the §6.2 regime-switched descent every JVM harness consumes — `descend` (coast 600 s → 800 km hand-off → terminal 30 s), `propagate_to_interception` → `Crossing`, `coast_to_handoff`, `apogee_state`, `earth_model`/`to_absolute_date`.
