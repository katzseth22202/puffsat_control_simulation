...
- `puffsat_sim/dispersion.py` — pure MC core: `DispersionSpec`, `RunInputs`, `sample_run_inputs`, `replay_inputs` (§14.2 seed-replay), RTN math, `summarize`, `EnsembleStats` (no JVM).
...
- `puffsat_sim/montecarlo.py` — JVM-side dispersion harness: `run_ensemble` with the `control` hook (open-loop `control=None`, or the Rung A1 corrector via predict/execute) and an optional `sink_path` for checkpoint/resume; `make capstone` (ADR 0002, 0003). C1 seam: `_truth_arc_to_apogee` (full-force arc ending at the apogee node) + `run_nav_feasibility` + `nav_feasibility_report` (measured Φ → envelope → NEES) (ADR 0012). C2a seam: `coeff_requirement_report` (measured Φ + two 1D A3 cuts → tolerance-vs-prior verdict) (ADR 0013). C3a seam: `run_terminal_feedforward` + `terminal_feedforward_report` (equivalence-pin the fixed-step Cowell terminal vs adaptive-30s unburned, sample the drag profile, execute the ZOH plan as maneuver segments, measure the residual vs the drag-free reference) (ADR 0014).
