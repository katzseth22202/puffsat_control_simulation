...
- `puffsat_sim/montecarlo.py` — JVM-side dispersion harness: `run_ensemble` with the `control` hook (open-loop `control=None`, or the Rung A1 corrector via predict/execute) and an optional `sink_path` for checkpoint/resume; `make capstone` (ADR 0002, 0003). C1 seam: `_truth_arc_to_apogee` (full-force arc ending at the apogee node) + `run_nav_feasibility` + `nav_feasibility_report` (measured Φ → envelope → NEES) (ADR 0012).
