...
- **B0 — terminal-propagation regime-switch (infrastructure; do first).** The §6.2 fix: coast on the big adaptive step, hand off at an altitude event to a fixed-step Cowell terminal phase. The interim 30 s step-cap is a **~5–6× tax** per descent on `full_force` with no accuracy gain (profiled: identical perigee/ToA at 30 vs 120–600 s caps); regime-switching removes it *and* gives B3's continuous burn a stable terminal phase. Check: dispersed `full_force` descents reach the 200 km event without overshoot, faster, same perigee. (Process-level multiprocessing stays a **Rung D** tool, §11.2/§12 — it only parallelizes the multi-scenario axis, and the ~40 s per-worker JVM cold-start caps its small-N payoff.)
- **B1 — finite mass-depleting burn (the Actuator).** Map a commanded impulsive Δv (the *unchanged* A1 corrector) → a finite Orekit maneuver: 400 mN, Isp, Tsiolkovsky mass depletion, ~1 °/s direction slew. Tracer check: a small apogee correction *executed* as a finite burn reproduces the impulsive A1 interception null and closes the propellant ledger. **This deliberately breaks the A-rung `predict == execute` identity** (the corrector predicts impulsive, truth executes finite); the residual miss *is* the actuator-realism erosion B1 measures — expected small at apogee, a finding if not. A finite-burn-aware targeter is deferred (ADR 0008).
- **B3 — feedforward terminal anti-drag burn (actuator authority).** Under perfect state the apogee burn already nulls the aim including the *known* drag (A3), so the terminal burn is a deliberate propellant-suboptimal **cost baseline**, not a closed loop (that is C/D's job). Instrument the known-drag descent 600 → 200 km: anti-drag Δv = ∫(drag accel)·dt, peak required thrust = max │drag force│, peak slew rate. Deliverable: confirm peak thrust ≤ 400 mN and slew ≤ 1 °/s (the ~10× margin — a result to confirm, not assume), plus the anti-drag Δv. Needs B0's terminal phase.
- **B2 — propellant ledger + Isp sweep (the headline deliverable).** Aggregate the total Δv (B1 corrections + B3 anti-drag, the latter dominating) → propellant fraction = Δv/(Isp·g₀) at Isp ∈ {50, 70, 200} s → the fraction-vs-Isp curve against the <2%-of-25 kg line. Mostly pure post-processing (reuses the ADR 0004 linear model / the A3 `budget_dv_m_s` helper); expect the 50 s anchor to fail the claim ~3×. This is Rung B's test of the paper's propellant claim.

Order: B0 → B1 → B3 → B2. Dead-time, MIB / cluster geometry, cosine losses, the coarse/fine chemical+cold-gas split, and the cm-aim 100 Hz trim remain deferred (ADR 0004).
