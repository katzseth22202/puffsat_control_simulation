...
- **A2 — two-burn statistical midcourse.** Add a second coast correction to A1's single apogee impulse: correction 1 near apogee, correction 2 at a fixed mid-descent altitude (**900 km**, §16.6 band), both impulsive and perfect-state. Schedule is **fixed and always-executed** (§16.6); threshold/adaptive triggering is deferred to Rung D. **Settled 2026-06-09 (ADR 0005):**
  - **Two burns, no terminal aim at Rung A.** The §9 "terminal aim" is *vestigial* under perfect state + perfect model — converged coast burns null the 200 km crossing to machine precision, so an impulsive terminal aim solves for identically zero. It becomes real as a *finite* drag-rejecting burn at Rung B. A2 is exactly two impulses (6 DOF).
  - **Surplus authority spent on Δv efficiency.** Two burns give 6 DOF against the same 3 position constraints, so the solve is underdetermined; A2 resolves it as a **minimum-Δv least-norm Gauss-Newton** (`pinv`/`lstsq` on a 3×6 finite-difference Jacobian — one line off A1's square `solve`, still pure-numpy; scipy stays deferred to A3, which is *authority-boundary* robustness, a separate concern). This directly attacks the A1 carry-forward: the radial-injection runs that cost a single apogee burn ~88 m/s collapse to a cheap two-burn split.
  - **Segmented event-restart for burn 2** (not Orekit `ImpulseManeuver`, per the §4 architecture): propagate apogee → 900 km descending altitude event (stop) → re-derive the RTN basis from the node state → apply burn 2 → restart → 200 km crossing. The node-state-dependent basis keeps the corrector a pure black-box-propagator method (ADR 0003 decision 5); minimizing Σ‖Δv‖² is frame-invariant because each burn's RTN frame is orthonormal. A1's `max_step` cap carries over.
  - **Deliverable: same-seed A2-vs-A1.** Run identical `DispersionSpec`/`master_seed` under `control=None`, A1, and A2; A2's result is the Δv ledger (mean/max), the **per-burn split** (does the mid-descent burn earn its place?), and the **convergence-fraction gain** (does the two-burn solve converge where A1 hit the 88 m/s wall?). "Few km of residual reaches the terminal phase" is a Rung C/D statement — at Rung A it is ~0 for converged runs, so it is **not** an A2 criterion. No total-Δv budget gate in the solver: mapping where Δv exceeds budget is the **A3** controllability map.
  - **Acceptance:** converged runs land within `tol_m` of the nominal crossing; propellant ledger closes; the 800/600 km regime hand-offs and eclipse events fire correctly (propagation correctness — no burn there).
...
- A2's two-burn correction nulls the crossing at **lower total Δv** than A1's single apogee burn on the same seeds, and converges on runs A1 cannot (the along-track/phase carry-forward); the per-burn split and convergence fraction are recorded results, not pass/fail (ADR 0005).
